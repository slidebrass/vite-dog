...
export const token = 'live_sM7CeCWZm4IZGKFZ0MnVD7MF3ijTrjzaSQrIGtEZGbbpTGei1L8SYYPbgwA935t7'

export const dog_server_calls = {
  
  get: async (breed_name: string, image_count: number) => {
    const response = await fetch(`https://api.thedogapi.com/v1/images/search?limit=${image_count}&breed_ids=${breed_name}&?api_key=${token}`,
    {
      method: 'GET',
      headers: {
